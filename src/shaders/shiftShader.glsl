...
void main() {
  ...
  vec2 vUvNormal = vec2( vUv - 0.5 );
  vec2 vUvMouse = vec2( vUv.x - u_mouse.x, vUv.y - u_mouse.y);
  vec2 dxy = pixelSize * 10./ resolution;
  float circle = smoothstep(
    0.2,
    1.2,
    dot(vUvNormal, vUvNormal) * 2.
  );
  ...
  gl_FragColor = texture2D( tDiffuse, vUv + ( pixelated - 0.5 )* ( circle ) * ( 1. - length(u_mouseSpeed) * 3.5 ) ); //somehow working
